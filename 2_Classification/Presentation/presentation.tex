...
% SVC: support vector classifier, no kernel, no transformation of data
% SVM: support vector machines, with kernel and transformation, then the SVC can
% split the data
...
% kernel functions: systematically find SVCs in higher dimensions
% example polynomial kernel: increases dimensions by changing the degree and
% calculating the relation between all data points to find a SVC,
% cross-validation: find good value for d
% example: radial basis function (RBF) kernel, finds SVCs in infinite dimensions

% polynomial kernel: (a \times b + r)^d, a, b are observations, r is the
% coefficient, d the degree
% get high dimensional relationships by just plugging the observations into the
% kernel, the transformation is not needed

% RBF kernel: \exp(-\gamma (a-b)^2): behaves like a weighted nearest neighbors
% model
% squared distance, so influence is function of that and \gamma scales the
% influence (the larger, the less influence)
% infinite because exponential function can be expanded into an infinite taylor
% series

% kernel trick: calculate the relation between different features as if they
% were in the higher dimension, transformation doesn't actually happen, makes
% the infinite dimensions of the RBF kernel possible

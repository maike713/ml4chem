...
\begin{frame}{Probabilities and Notation~\cite{wikipedia-bayes-en}}
    \begin{columns}
...
\begin{frame}{Bayes' Theorem~\cite{wikipedia-bayes-en}}
    \begin{columns}
...
\begin{frame}{Bayes' Theorem -- Example~\cite{wikipedia-bayes-de}}
...
\begin{frame}{Linear Regression~\cite{Hastie-2009}}
    \begin{columns}
        \column{\kitthreecolumns}
            \begin{itemize}
...
\begin{frame}{Linear Regression~\cite{Hastie-2009}}
    \begin{columns}
        \column{\kitthreecolumns}
        \begin{greenblock}{Unique solution for the parameters:}
        \begin{equation}
    ...
    \end{greenblock}
        ...
        \begin{itemize}
            \setlength\itemsep{2em}
            \item Features $X$: medical information (ten columns)
            \item Target $Y$: diabetes or not
            \item $\boldsymbol{X}$: $(N_\text{test} \times 11)$, first column
                consisting of ones 
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Linear Regression -- Exercise}
    \begin{itemize}
        ...
        \item Calculate $\hat{\beta}$ with the prepended training matrix and the
            targets: \\
            Matrix multiplication: $\Rightarrow$ Numpy \texttt{np.matmul} \\
        Invert matrix: $\Rightarrow$ \texttt{np.linalg.inv} or \texttt{@} \\
